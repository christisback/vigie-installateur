...
OutputDir=..\output
OutputBaseFilename=Vigie-Billets-MiseAJour
...
[Files]
; Version allégée réservée à la mise à jour d'un poste où Node.js et PostgreSQL
; sont déjà installés — exclut les deux gros installateurs (33 Mo + 376 Mo) que
; setup-app.ps1 ignore de toute façon quand il les détecte déjà présents.
; Pour une toute première installation, utiliser Vigie-Billets-Installateur.exe.
Source: "payload\*"; DestDir: "{app}"; Flags: recursesubdirs ignoreversion; Excludes: "_setup\node-v24.19.0-x64.msi,_setup\postgresql-18.6-1-windows-x64.exe"
...
Filename: "powershell.exe"; \
  ...
  StatusMsg: "Mise à jour de Vigie Billets — cela peut prendre quelques minutes..."; \
...
begin
  ...
  begin
    MsgBox('Aucune installation existante de {#MyAppName} détectée sur ce poste.' + #13#10#13#10 +
      'Cet installateur de MISE À JOUR suppose que Node.js et PostgreSQL sont déjà installés.' + #13#10 +
      'Pour une première installation, utilisez plutôt Vigie-Billets-Installateur.exe (version complète).',
      mbError, MB_OK);
    Result := False;
    Exit;
  end;
  ...
  begin
    Msg := '{#MyAppName} version ' + InstalledVer + ' est installé. Cette mise à jour contient la version {#MyAppVersion}.';
